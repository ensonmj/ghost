...
type Socks5HandleServer struct {
...
func (s *Socks5HandleServer) ServeSocks5(c net.Conn) {
	log.Printf("[socks5] %s -> %s\n", c.RemoteAddr(), c.LocalAddr())
	conn := gosocks5.ServerConn(c, s.selector)
	req, err := gosocks5.ReadRequest(conn)
	if err != nil {
		log.Printf("[socks5]: %s\n", err)
		return
	}
	log.Printf("[socks5] %s -> %s\n%s\n", c.RemoteAddr(), req.Addr, req)
...
func (s *Socks5HandleServer) handleConnect(c net.Conn, req *gosocks5.Request) {
	cc, err := s.Dial("tcp", req.Addr.String())
	if err != nil {
